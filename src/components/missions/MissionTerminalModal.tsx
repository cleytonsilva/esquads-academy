...
export default function MissionTerminal({ 
  ...
  // Steps com fallback em metadata
  const missionSteps = mission?.steps || mission?.metadata?.steps

  useEffect(() => {
    if (mission) {
      // Inicializar com código template se disponível (fallback em metadata)
      const tmpl = mission.template_code || mission.metadata?.template_code
      if (tmpl) {
        setCode(tmpl)
      }
      ...
      if (result.success) {
        ...
        if (result.validation) {
          ...
          if (result.validation.passed) {
            setCurrentStep(prev => Math.min(prev + 1, missionSteps?.length || 0))
          }
        }
      } else {
  ...
  const resetCode = () => {
    setCode(mission?.template_code || mission?.metadata?.template_code || '')
  }
          ...
          {missionSteps && (
            <div className="space-y-2">
              <div className="flex justify-between text-sm text-gray-600">
                <span>Progresso</span>
                <span>{currentStep}/{missionSteps.length} etapas</span>
              </div>
              <Progress value={(currentStep / missionSteps.length) * 100} className="h-2" />
            </div>
          )}
        </DialogHeader>
